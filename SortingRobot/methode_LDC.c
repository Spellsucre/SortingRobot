...
CelluleLDC* creerCellule(int i, int j){
    ...
    if (cel == NULL){ printf("Erreur lors de l'allocation"); return NULL; }
...
int LDCVide(LDC *ldc){
    if(ldc == NULL){ printf("Liste non existante"); return 0; }
...
void LDCInsererEnFin(LDC *ldc, int i, int j){
    if(ldc == NULL){ printf("Liste à initialiser avant d'insérer!"); return; }
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// Enleve la première occurence de la cellule (à savoir celle qui a les même
// coordonnées, une seule dans toute la matrice)
void LDCEnleverCellule(LDC* ldc, CelluleLDC *cel){
    if (ldc == NULL){ printf("Liste non existante"); return; }
...
CelluleLDC* LDCrechercherPlusProcheCase(LDC* ldc, int i, int j){
    if (ldc == NULL){ printf("Liste non existante"); return NULL; }
    if (LDCVide(ldc)){ printf("Liste vide"); return NULL; }
...
void algorithme_parcouleur(Grille *G, Solution *S){
    ...
    //parcours de la grille, remplissage de TC avec les ldc correspondantes
    for (m=0; m<G->m; m++){
		for (n=0; n<G->n; n++){
